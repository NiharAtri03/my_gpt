...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Hint: Make sure you properly handle shift-by-1.
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, seq_len):
        ...
        ### TODO: You will need to complete the following
        # Compute the t in sin(t) and cos(t), using the position ids (position) and rotation frequencies (inv_freq)
        # Shape: (seq_len, dim/2)
        # don't modify in place
        inv_freq = self.inv_freq 
        position = torch.arange(seq_len, device=inv_freq.device).unsqueeze(1)
        theta = position * inv_freq.unsqueeze(0) 
...
def apply_rotary_emb(q, k, sin, cos):
    ...
    def rotation(x, sin_value, cos_value):
        x_even = x[:,:,:, 0::2]
        x_odd = x[:,:,:, 1::2]

        sin_value = sin_value.unsqueeze(0).unsqueeze(0)
        cos_value = cos_value.unsqueeze(0).unsqueeze(0)

        rotated_even = x_even * cos_value - x_odd * sin_value
        rotated_odd = x_even * sin_value + x_odd * cos_value
    ...
    if q.size(2) != 1:
        ...
        q_rotated = rotation(q, sin[-1:,:], cos[-1:,:])
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None, use_cache=False):
        ...
        combined_matrix = self.c_attn(x)
        q, k, v = combined_matrix.split(C, dim=2)
        nh = self.n_head
        hs = self.split_size

        
        # Reshape q, k, v to (B, nh, T, hs)
        # TODO: need to reshapre before tranposing to split C dimensions
        q = q.view(B, T, nh, hs)
        k = k.view(B, T, nh, hs)
        v = v.view(B, T, nh, hs)
        ...
        sin, cos = self.rotary_emb.forward(seq_len)
        ...
        if layer_past is None:
            att_scores = torch.where(self.mask[:, :, :T, :T] == 1, att_scores, torch.full_like(att_scores, float('-inf')))
